...
> Automated microbiological report generation for food safety labs — powered by Claude AI, validated against FDA/FSMA regulatory limits, enriched with live FDA recall intelligence.

---

## What it does

Food safety labs generate dozens of microbiological reports per week. Each one requires a trained microbiologist to manually interpret raw counts, cross-reference regulatory limits, write corrective actions, and format a compliance document. That process takes 45–60 minutes per report.

This pipeline does it in 30 seconds.

A lab uploads a CSV of raw results. The pipeline validates each result against FSMA and FDA limits, calls the Claude API to generate a plain-language interpretation, pulls live recall data from the FDA Recall Enterprise System, and outputs a professional audit-ready PDF — complete with risk classification, corrective actions, CCP recommendations, and official FDA recall citations.
...
**Live FDA recall intelligence** — queries the openFDA Food Enforcement API in real time. For each pathogen detected, the report shows total recalls on record, recalls in the last 24 months, most recent recall with product description and date, recall classification, average recall cost, and official FDA recall number for verification.
...
├── demo_results_..._report.pdf   # Sample generated report
...
Built as a side project during my PhD in Microbiology at UC Merced (expected graduation 2026). The Central Valley produces ~25% of US food supply — FSMA compliance and microbiological testing are daily realities for hundreds of facilities within 60 miles of campus.

The tool was designed with three constraints in mind: no certification required to use it, no client data stored or retained, and output that a non-technical manager can understand in 30 seconds.
...
*This tool is intended for use as a decision-support aid. All reports should be reviewed by a qualified microbiologist before regulatory action. Input data is processed locally and not stored or retained.*